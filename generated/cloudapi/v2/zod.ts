...
export const ComposeStatusValue = z.enum(['success', 'failure', 'pending']);

export const ImageStatusValue = z.enum([
  'success',
  'failure',
  'pending',
  'building',
...
export const UploadStatusValue = z.enum([
  'success',
  'failure',
  'pending',
  'running',
...
export const AWSEC2UploadStatus = z
...
export const ContainerUploadStatus = z.object({
  url: z.string(),
  digest: z.string(),
});
...
export const PulpOSTreeUploadStatus = z
  .object({ repo_url: z.string() })
  .passthrough();

export const LocalUploadStatus = z
  .object({ artifact_path: z.string() })
  .passthrough();
...
export const ImageStatus = z
  .object({
    ...
    upload_statuses: z.array(UploadStatus).optional(),
    error: ComposeStatusError.optional(),
  })
  .passthrough();

export const KojiStatus = z
  .object({ build_id: z.number().int() })
  .partial()
  .passthrough();

export const ComposeStatus = ObjectReference.and(
  z
    .object({
      status: ComposeStatusValue,
      image_status: ImageStatus,
      image_statuses: z.array(ImageStatus).optional(),
      koji_status: KojiStatus.optional(),
    })
    .passthrough(),
);

export const ComposeList = List.and(
  z.object({ items: z.array(ComposeStatus) }).passthrough(),
);

export const ComposeDeleteStatus = ObjectReference;

export const PackageMetadataCommon = z
  .object({
    type: z.string(),
    name: z.string(),
    version: z.string(),
...
export const PackageMetadata = PackageMetadataCommon.and(
  z.object({ sigmd_5: z.string() }).passthrough(),
);
...
export const ComposeMetadata = ObjectReference.and(
  z
    .object({
      packages: z.array(PackageMetadata),
      ostree_commit: z.string(),
...
export const KojiLogs = z.object({}).passthrough();

export const ComposeLogs = ObjectReference.and(
  z
    .object({
      image_builds: z.array(z.object({}).partial().passthrough()),
      koji: KojiLogs.optional(),
    })
    .passthrough(),
);

export const ComposeManifests = ObjectReference.and(
  z
    .object({ manifests: z.array(z.object({}).partial().passthrough()) })
    .passthrough(),
);

export const ImageSBOM = z
  .object({
    sbom: z.object({}).partial().passthrough(),
    sbom_type: z.literal('spdx'),
    pipeline_name: z.string(),
    pipeline_purpose: z.enum(['buildroot', 'image']),
  })
  .passthrough();

export const ComposeSBOMs = ObjectReference.and(
  z.object({ items: z.array(z.array(ImageSBOM)) }).passthrough(),
);

export const AWSEC2CloneCompose = z.object({
  region: z.string(),
  share_with_accounts: z.array(z.string()).optional(),
});

export const CloneComposeBody = AWSEC2CloneCompose;

export const CloneComposeResponse = ObjectReference.and(
  z.object({ id: z.string().uuid() }).passthrough(),
);

export const CloneStatus = ObjectReference.and(UploadStatus);

export const ComposeId = ObjectReference.and(
  z.object({ id: z.string().uuid() }).passthrough(),
);

export const DepsolveRequest = z.object({
...
export const DepsolveResponse = z
  .object({ packages: z.array(PackageMetadataCommon) })
  .passthrough();

export const SearchPackagesRequest = z.object({
  repositories: z.array(Repository).optional(),
  distribution: z.string(),
  architecture: z.string(),
  packages: z.array(z.string()),
...
export const SearchPackagesResponse = z
  .object({ packages: z.array(PackageDetails) })
  .passthrough();

export const DistributionList = z.record(
  z.record(z.array(BlueprintRepository)),
);

export const ErrorList = List.and(
  z.object({ items: z.array(Error) }).passthrough(),
);
